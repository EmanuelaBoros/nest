...
public class MatcherTest {
    ...
    private final static String inputSARpath2 = "SAR/"
            + "ASA_WSM_1PNUPA20080508_101559_000000672068_00237_32355_1989.N1";
    EnvisatProductReaderPlugIn readerPlugIn = new EnvisatProductReaderPlugIn();
    EnvisatProductReaderPlugIn readerPlugIn2 = new EnvisatProductReaderPlugIn();
...
//    @Test
    public void testComputeMatchScore() throws Exception {
        ...
        float result = 0f;
        float fResult = instance.computeMatchScore(sourceBand, sourceTile,
                sourceBand, templateTile,
    ...
    public void testComputeSIFTMatchScore() throws Exception {
        ...
        final EnvisatProductReader reader = (EnvisatProductReader) readerPlugIn.createReaderInstance();
        final EnvisatProductReader reader2 = (EnvisatProductReader) readerPlugIn2.createReaderInstance();

        URL url = getClass().getClassLoader().getResource(inputSARpath);
        URL url2 = getClass().getClassLoader().getResource(inputSARpath2);

        final Product sourceProduct = reader.readProductNodes(url.getFile(), null);
        final Product targetProduct = reader2.readProductNodes(url2.getFile(), null);
        assertNotNull(sourceProduct);

        final BasicThresholdingOp op = (BasicThresholdingOp) spi.createOperator();
        final BasicThresholdingOp op2 = (BasicThresholdingOp) spi.createOperator();
        assertNotNull(op);
        op.setSourceProduct(sourceProduct);
        op2.setSourceProduct(targetProduct);
        Band sourceBand = sourceProduct.getBandAt(0);
        ...
        final Rectangle sourceTileRectangle = getSourceTileRectangle(2900, 4000, 400, 400);
        final Rectangle templateRectangle = getSourceTileRectangle(2700, 4000, 400, 400);

        Tile sourceTile = op.getSourceTile(sourceBand, sourceTileRectangle);
        Tile templateTile = op2.getSourceTile(targetBand, templateRectangle);

        SIFTMatcher instance = new SIFTMatcher();
        float fExpResult = 0f;
        float fResult = instance.computeMatchScore(sourceBand, sourceTile,
                targetBand, templateTile,
                Mode.SUM_SQUARED_DIFFERENCE);
        assertEquals(fExpResult, fResult, 195.27601623535156f);
    }
